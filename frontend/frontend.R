...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    width = 250,
    sidebarMenu(
      menuItem("Vulnerability Map", tabName = "vulnerability_map", 
               icon = icon("exclamation-triangle")),
      menuItem("Connectivity Map", tabName = "connectivity_map", 
  ...
  dashboardBody(
    ...
    tabItems(
      tabItem(tabName = "vulnerability_map",
              fluidRow(
      ...
      tabItem(tabName = "connectivity_map",
      ...
      tabItem(tabName = "analysis",
              fluidRow(
                box(width = 6,
                    title = "Top 5 Most Vulnerable Stations",
                    DTOutput("top_vulnerable_table2")),
                box(width = 6,
                    title = "Top 5 Least Connected Stations",
                    DTOutput("low_connectivity_table"))
              )
...
server <- function(input, output, session) {
  ...
  ###TOP 5###
  #action button updates the table (together with the map)
  reactive_vul_data <- eventReactive(input$update, {
    req(input$day_of_week, input$peak_bool)
    
    v_df %>% 
      filter(day_type == input$day_of_week & is_peak == input$peak_bool) %>%
      arrange(desc(avg_score))
  })
  
  # Reactive expression for connectivity data (replace with your actual connectivity logic)
  reactive_conn_data <- reactive({
    req(input$day_of_week, input$peak_bool)
    
    # Replace this with your actual connectivity data filtering
    connectivity_data_map %>%
      filter(day_type == input$day_of_week & is_peak == input$peak_bool) %>%
      arrange(Score)
  })
  
  # Top Vulnerable Stations Table
  output$top_vulnerable_table1 <- renderDT({
    data <- reactive_vul_data() %>%
      head(5) %>%
      select(station, station_code, avg_score) %>%
  ...
  output$top_vulnerable_table2 <- renderDT({
    data <- reactive_vul_data() %>%
      head(5) %>%
      select(station, station_code, avg_score) %>%
      rename("Station" = station,
  ...
  # Least Connected Stations Table
  output$low_connectivity_table <- renderDT({
    data <- reactive_conn_data() %>%
      head(5) %>%
